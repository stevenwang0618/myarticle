...
\begin{abstract}
Rubber balloon is inflated by the pressure jump across the membrane due to inner gas.
If the inner gas is lighter than air, the rubber balloon will produce buoyance to lift a payload.
So the large rubber balloon can serve as a vehicle to transport scientific or military apparatus into high altitude.
For rubber is material of entropy induced elasticity, the deformation of rubber balloon depends on temperature.
The environmental temperature varies along altitude and changes significantly during the diurnal cycles, it is significant to study the thermoelastic effect of rubber on the ascending process of rubber balloon.

In this paper, we proposed a modified Mooney-Rivlin model of rubber which considered the dependence of elastic coefficients on temperature. We studied the ascending process of rubber balloon with payload using this modified model and compared its results with the normal one. We also found that the rubber balloon would keep its shape at high altitude during the diurnal cycles, which means the balloon could maintain nearly a constant altitude for a long period. The results given here show the temperature dependence of elasticity is not negligible and can be wisely utilized when we use rubber balloon as High Altitude Long Duration(HALD) aerostat.
\\
\emph{Key words}: Rubber balloons, Mooney--Rivlin model, thermoelasticity, HALD aerostat
\end{abstract}
...
In fact the coefficients of elasticity in Mooney-Rivlin model depend on temperature due to its entropy induced elasticity\cite{Ingo}, this may influence the performance of rubber balloon.

In this paper, we proposed a modified Mooney-Rivlin model which considered the temperature dependence of rubber's elasticity. We studied the temperature effect of rubber's elasticity on the ascending process of rubber balloon. The results shows temperature has a significant effect on the performance of rubber balloon especially at high altitude. Besides, we also found rubber balloon will keep its shape at high altitude when temperature changes, this means that the rubber balloon could stay aloft at a constant altitude over many diurnal cycles.
...
\section{Mooney-Rivlin model}
 ...
 \begin{equation}\label{prrelation}
  \Delta p(r)=2s_+\frac{t_0}{r_0}\left(\frac{r_0}{r}-\left(\frac{r_0}{r}\right)^7\right)\left(1-\frac{s_-}{s_+}\left(\frac{r}{r_0}\right)^2\right)
 \end{equation}
 ...
 \begin{equation}\label{total}
  p_{out}(h)-\frac{n R T(h)}{\frac{4}{3} \pi r^3}+2s_+\frac{t_0}{r_0}\left(\frac{r_0}{r}-\left(\frac{r_0}{r}\right)^7\right)\left(1-\frac{s_-}{s_+}\left(\frac{r}{r_0}\right)^2\right)=0
 \end{equation}
...
\subsection{Thermoelasticity}
 ...
 \begin{equation}\label{total1}
   p_{out}-\frac{n R T}{\frac{4}{3}\pi r^3}+2s_+(T)\frac{t_0}{r_0}\left(\frac{r_0}{r}-\left(\frac{r_0}{r}\right)^7\right)\left(1-\frac{s_-(T_0)}{s_+(T_0)}\left(\frac{r}{r_0}\right)^2\right)=0
 \end{equation}
...
\end{equation}
Figure \ref{RA} shows the resulting graph of balloon radius versus height. As well as the Mooney-Rivlin model ($s_-$ and $s_+$ are constant during ascending process) and the modified Mooney-Rivlin model($s_-$ and $s_+$ change with altitude during ascending process), the graph also includes a purely theoretical non-restoring model, in which the balloon exerts no retractive elastic force at all ($\Delta p=0$, so the inner pressure is always equal to the atmospherical pressure).
\begin{figure}[!hbt]
  \centering
  % Requires \usepackage{graphicx}
...
\begin{equation}\label{total2}
   - \frac{{n{R}T}}{{\frac{4}{3}\pi {r^3}}} + 2{s_ + }(T)\frac{{{t_0}}}{{{r_0}}}\left(\frac{{{r_0}}}{r} - {\left(\frac{{{r_0}}}{r}\right)^7}\right)\left(1 - \frac{{{s_ - }({T_0})}}{{{s_ + }({T_0})}}{\left(\frac{r}{{{r_0}}}\right)^2}\right
) = 0
\end{equation}
we can cancel $T$ in each term, then we get:
\begin{equation}\label{total3}
   - \frac{{n{R}}}{{\frac{4}{3}\pi {r^3}}} + 2\frac{{{s_ + }({T_0})}}{{{T_0}}}\frac{{{t_0}}}{{{r_0}}}\left(\frac{{{r_0}}}{r} - {\left(\frac{{{r_0}}}{r}\right)^7}\right)\left(1 - \frac{{{s_ - }({T_0})}}{{{s_ + }({T_0})}}{\left(\frac{r}{{{r_0}}}\right)^2}\right) = 0
\end{equation}

It suggests that the balloon's radius $r$ is independent with temperature, the balloon will keep its shape when the balloon's temperature changed, this is important when we consider a balloon that is capable of staying aloft for half a year at high altitude. At night the temperature of the lifting gas cools, while the temperature rises caused by solar heating during the day. These are called ``supercool" and ``superheat". From the equation above, though the temperature varies during the diurnal cycles, the balloon will keep its volume, so the residence altitude. For some aerostats need carry a significant amount of ballast to maintain constant altitude over several diurnal cycles, it will be a great advantage when we use rubber balloon as the High Altitude Long Duration aerostat.


\section{Conclusion}
The ascending process of rubber balloon has been studied. Due to the entropy induced elasticity, the elastic behavior of rubber varies at different temperature. We set rubber as material of a modified Mooney-Rivlin relation, and we found that the temperature effect of elasticity plays an important role in ascending process.
We also show the interesting phenomenon that the balloon will keep its height when temperature changes at high altitude. This fact may be wisely utilized when we develop a High Altitude Long Duration aerostat.

But the dependence of elastic coefficients of rubber on temperature is rather complex, and the nature of them is still not clear\cite{Shedd Ingersol}. In fact, from the experiment the coefficient $s_-$ is always increasing more slowly than the absolute temperature\cite{Shumskaya}. Even more, the material of rubber balloons is not strictly a Mooney-Rivlin model. All these make it more difficult to predict the performance of rubber balloon, which needs more examination.
