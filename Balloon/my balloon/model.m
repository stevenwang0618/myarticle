% balloon parameters
r0=0.5; % uninflated radius (m)
initial=1; % initial radius (m)
burst=5.0; % burst radius (m)
d0=0.00025; % thickness (m)
mp=1.5; % payload mass (kg)
...
mb=(4*pi*r0^2*rubberrho)*d0 % balloon mass (kg)
...
r3=radius_mooneyrivlin1(n,p,T,r0,d0,initial);
...
ascent_minutes=sum((h(2)-h(1))./v2(1:i2))/60
v2(1)
figure(1);
pl=plot(h(1:i1),r1(1:i1),'-',h(1:i2),r2(1:i2),'-.',h(1:i3),r3(1:i3),'--');
%set(pl, 'linewidth', 1.5);
title('Balloon radius with altitude');
legend('non-restoring model','Mooney-Rivlin model-modified','Mooney-Rivlin model','location','northwest');
xlabel('Altitude (m)');
